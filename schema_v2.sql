-- Haptic Sound Visualizer Research Database Schema v2 (SQLite)
-- Survey pair data: trial_survey_binary_actions, trial_survey_vibes
-- Legacy trial_survey_directions / trial_survey_textures are not used in v2.
...
CREATE TABLE trials (
    ...
    analyst_notes  TEXT,
    analyst_notes_updated_at DATETIME,
...
CREATE TABLE trial_survey_binary_actions (
...
CREATE TABLE trial_survey_vibes (
    response_id    INTEGER NOT NULL,
    pair_id        TEXT NOT NULL,
    value          TEXT NOT NULL,
    PRIMARY KEY (response_id, pair_id),
...
-- Analyst classification tag vocabulary. Seeded with Neutral / Attentive /
-- Disruptive; analysts can create more. A pattern may carry multiple tags.
...
CREATE INDEX idx_trial_survey_binary_actions_response ON trial_survey_binary_actions(response_id);
CREATE INDEX idx_trial_survey_vibes_response ON trial_survey_vibes(response_id);
